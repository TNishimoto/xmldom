...
wows.describe('errorHandle').addBatch({
	...
	'only function1': function () {
		...
		try {
			var doc = parser.parseFromString('<html disabled><1 1="2"/></body></html>', 'text/xml');
			error.map(function (e) { error[e.replace(/\:[\s\S]*/, '')] = e })
			console.assert(error.warning == `attribute "disabled" missed value!! "disabled" instead!!`, 'error.warning:' + error.warning);
			console.assert(error.error == "element parse error: Error: invalid tagName:1" , 'error.error:' + error.error);
			//console.assert(error.fatalError != null, 'error.fatalError:' + error.fatalError);
